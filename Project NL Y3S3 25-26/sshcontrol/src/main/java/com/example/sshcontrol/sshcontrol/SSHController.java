...
public String listServices(Model model, HttpSession session) {
    ...
    // Lấy danh sách tất cả dịch vụ trên máy chủ
    String result = sshService.executeCommand(
        host,
        username,
        password,
    ...
    // Tách từng dòng dịch vụ thành mảng
    ...
    public String showControlService(Model model, HttpSession session) {
        ...
        String serviceListRaw = sshService.executeCommand(
            ...
            "systemctl list-units --type=service --no-legend --all | awk '{print $1}'"
        );
    ...
    public String controlService(
        ...
        // Lấy lại danh sách dịch vụ cho dropdown
        String serviceListRaw = sshService.executeCommand(
            host, username, password,
            "systemctl list-units --type=service --no-legend --all | awk '{print $1}'"
        );
        ...
        // Nếu là start/stop và output rỗng thì báo thành công
        if ((action.equals("start") || action.equals("stop")) && (result == null || result.trim().isEmpty())) {
            model.addAttribute("result", "Thao tác " + action + " dịch vụ thành công!");
        } else {
            model.addAttribute("result", result);
        }
